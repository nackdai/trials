...
// 初期化.
IZ_BOOL MultiDrawApp::InitInternal(
...
void MultiDrawApp::createVBForDynamicStream(
    izanagi::IMemoryAllocator* allocator,
    izanagi::graph::CGraphicsDevice* device)
{
...
void MultiDrawApp::createVBForMapUnmap(
    izanagi::IMemoryAllocator* allocator,
    izanagi::graph::CGraphicsDevice* device)
{
...
void MultiDrawApp::createVertices()
{
...
// 解放.
void MultiDrawApp::ReleaseInternal()
{
#ifdef ENABLE_TRHEAD
    m_isRunThread = false;
...
// 更新.
void MultiDrawApp::UpdateInternal(izanagi::graph::CGraphicsDevice* device)
{
...
// 描画.
void MultiDrawApp::RenderInternal(izanagi::graph::CGraphicsDevice* device)
{
    RenderDynamicStream(device);
    //RenderMapUnmap(device);
}

void MultiDrawApp::RenderDynamicStream(izanagi::graph::CGraphicsDevice* device)
{
...
void MultiDrawApp::RenderMapUnmap(izanagi::graph::CGraphicsDevice* device)
{
    CALL_GL_API(::glEnable(GL_VERTEX_PROGRAM_POINT_SIZE));
    CALL_GL_API(::glEnable(GL_POINT_SPRITE));
...
IZ_BOOL MultiDrawApp::OnKeyDown(izanagi::sys::E_KEYBOARD_BUTTON key)
{
    return IZ_TRUE;
}
